...
-- (a) Competitor plus date range, pruned. The good path.
-- Expect: an Append node with a single child (raw_signals_2026_07), other months
-- absent, and an Index Scan using raw_signals_2026_07_comp_cap under it. The
-- predicate carries the partition key, so the planner prunes to one partition, then
-- the composite index resolves the competitor. One month touched, not three.
...
-- Scan here. Postgres falls back to the UNIQUE(competitor_id, content_hash,
-- captured_at) index, which also leads with competitor_id and carries captured_at, so
...
DROP INDEX raw_signals_2026_07_comp_cap;
...
CREATE INDEX raw_signals_2026_07_comp_cap
...
-- fans out across all months. Filtering on competitor_id alone cannot prune a table
-- partitioned by captured_at. Always carry a time bound, or partition by the axis
-- you actually filter on (HASH by competitor_id, see the guide's stretch goals).
EXPLAIN (ANALYZE, BUFFERS)
SELECT * FROM raw_signals WHERE competitor_id = 3;
...
CREATE INDEX IF NOT EXISTS raw_signals_2026_07_cover
  ON raw_signals_2026_07 (competitor_id, captured_at) INCLUDE (source_id);
VACUUM raw_signals_2026_07;
EXPLAIN (ANALYZE, BUFFERS)
SELECT competitor_id, captured_at, source_id FROM raw_signals
WHERE competitor_id = 3
  AND captured_at >= '2026-07-01' AND captured_at < '2026-08-01';
...
EXPLAIN (ANALYZE, BUFFERS)
SELECT * FROM daily_signal_rollup WHERE competitor_id = 3;

EXPLAIN (ANALYZE, BUFFERS)
SELECT competitor_id, date_trunc('day', captured_at) AS day,
       count(*), count(DISTINCT source_id)
FROM raw_signals WHERE competitor_id = 3
GROUP BY competitor_id, date_trunc('day', captured_at);

-- (f) Concurrent refresh. Refresh without locking readers.
-- Works only because the matview has a UNIQUE INDEX on (competitor_id, day). Without
