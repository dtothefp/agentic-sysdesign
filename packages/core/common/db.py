...
# the repo-root .env to the environment before any secret is read. Env-first, so compose/Railway
# values always win and prod (no .env on disk) is a no-op. This is what makes GROQ/ANTHROPIC/
# LANGSMITH keys work locally from the repo-root .env without a per-key loader.
