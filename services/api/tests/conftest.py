...
def api_client():
    ...
    # Import BEFORE popping the key: importing api.main runs load_local_env, which would
    # setdefault SYSDESIGN_API_KEY right back from the root .env if we popped first. Auth
    # reads os.environ per-request, so popping after import is what actually opens the routes.
